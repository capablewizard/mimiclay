...
PS
{
	...
	// in model space so the lumps are locked to the prop).
	float SlotDisplacement( float3 lp, float amp, float freq )
	{
		float3 x = lp * freq;
		float n = vnoise( x ) * 0.67 + vnoise( x * 2.03 + 11.7 ) * 0.33;
	...
	float SampleSlot(
		...
		float3 origin, float4 rot, float dispAmp, float dispFreq, float3 wp )
	{
		...
		if ( dispAmp > 0.0 )
			d -= SlotDisplacement( lp, dispAmp, dispFreq );
	...
	float SdfDistAll( float3 p )
	{
		float d = SampleSlot( g_tField0, g_tAtlas0, g_tIndirection0, g_nSdfSparse0,
			...
			g_vModelOrigin0, g_vModelRot0, g_flDispAmp0, g_flDispFreq0, p );
		...
		if ( g_nFieldCount > 1 )
			d = min( d, SampleSlot( g_tField1, g_tAtlas1, g_tIndirection1, g_nSdfSparse1,
				...
				g_vModelOrigin1, g_vModelRot1, g_flDispAmp1, g_flDispFreq1, p ) );
		...
		if ( g_nFieldCount > 2 )
			d = min( d, SampleSlot( g_tField2, g_tAtlas2, g_tIndirection2, g_nSdfSparse2,
				...
				g_vModelOrigin2, g_vModelRot2, g_flDispAmp2, g_flDispFreq2, p ) );
		...
		if ( g_nFieldCount > 3 )
			d = min( d, SampleSlot( g_tField3, g_tAtlas3, g_tIndirection3, g_nSdfSparse3,
				...
				g_vModelOrigin3, g_vModelRot3, g_flDispAmp3, g_flDispFreq3, p ) );
	...
	float4 MainPs( PixelInput i, bool isFrontFace : SV_IsFrontFace ) : SV_Target0
	{
		...
		float L = 4.0 * max( max( g_flDispAmp0 * g_flDispFreq0, g_flDispAmp1 * g_flDispFreq1 ),
		                     max( g_flDispAmp2 * g_flDispFreq2, g_flDispAmp3 * g_flDispFreq3 ) );
